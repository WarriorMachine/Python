...
def maj():
    ...
    connect.set(plug),perct.set(str(battery.percent)+"%"),timer.set(time.strftime('%H:%M:%S'))
    main.after(1000, maj)
...
plugtext.place(x=ll/1.5, y=0, width= ll-ll/1.5, height= 12)
Button(main, textvariable=timer, bg='#1a1a1a', fg='gray60', border=0, activebackground="#1a1a1a", activeforeground="#1a1a1a", command= lambda:moreLess(False)).place(x=0, y=15, width= ll, height= 10)
Button(main, text="❌", bg='#1a1a1a', fg='#1a1a1a', activebackground='crimson', border=0, command=quit).place(x=ll-7, y=0, width= 7, height= 7)
